...
*/
  ...
  /*
      sql("create table srcs(key INT, value STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' ")
      sql("LOAD DATA INPATH '/user/aa.a' INTO TABLE srcs")

      // Queries are expressed in HiveQL
      println("Result of 'SELECT *': ")
      sql("SELECT * FROM srcs").collect().foreach(println)

      // Aggregation queries are also supported.
      val count = sql("SELECT COUNT(*) FROM srcs").collect().head.getLong(0)
      println(s"COUNT(*): $count")

      // The results of SQL queries are themselves RDDs and support all normal RDD functions.  The
      // items in the RDD are of type Row, which allows you to access each column by ordinal.
      val rddFromSql = sql("SELECT key, value FROM srcs WHERE key < 10 ORDER BY key")
      ...
      println("Result of SELECT *:")
      sql("SELECT * FROM records r JOIN srcs s ON r.key = s.key").collect().foreach(println)
  */
    sc.stop()
  }
}
